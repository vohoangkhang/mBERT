...
def create_sequences(data, seq_length):
    ...
    for i in range(len(data) - seq_length):
        ...
        y.append(data[i + seq_length, 3]) # Close price is at index 3 (standardized)
    return np.array(x), np.array(y)

def train_and_eval_mbert(ticker, file_path, notebook_dir):
    print(f"\n==================== Training for {ticker} (mBERT Features) ====================")
    ...
    features = ['Open', 'High', 'Low', 'Close', 'Volume',
                ...
                'volume_mentions', 'weighted_sentiment_index', 'sentiment_p_n_score', 'temporal_factor']
    
    # Ensure all features exist
    available_features = [f for f in features if f in df.columns]
    print(f"[{ticker}] Using features: {available_features}")
    ...
    if len(test_data) <= seq_length:
        print(f"  ! Dữ liệu quá ngắn cho {ticker}, bỏ qua.")
        return None
    ...
    # --- XGBoost ---
    print(f"[{ticker}] Training XGBoost...")
    ...
    plt.plot(y_pred_xgb_scaled, label=f'XGBoost mBERT (R2: {r2_xgb:.3f})', alpha=0.8, linestyle='--')
    plt.plot(y_pred_bilstm_scaled, label=f'BiLSTM mBERT (R2: {r2_bilstm:.3f})', alpha=0.8)
    plt.title(f"Stock Price Prediction with mBERT Sentiment & 8 Attributes - {ticker}")
    plt.xlabel("Time Steps (Test Set)")
    plt.ylabel("Normalized Price")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(notebook_dir, f"{ticker}_mbert_results.png"))
    plt.close()
...
if __name__ == "__main__":
    ...
    # Files to process
    file_map = {
        "AAPL": "AAPL_final_8_features_mbert.csv",
        "AMZN": "AMZN_final_8_features_mbert.csv",
        "GOOGL": "GOOGL_final_8_features_mbert.csv",
        "VNM": "VNM_final_compare_mbert_test.csv"
    }
    
    all_res = {}
    for t, f in file_map.items():
        file_p = os.path.join(processed_dir, f)
        if os.path.exists(file_p):
            res = train_and_eval_mbert(t, file_p, notebook_dir)
            if res: all_res[t] = res

    # Save metrics report
    report_path = os.path.join(processed_dir, "model_metrics_mbert.txt")
    with open(report_path, "w", encoding='utf-8') as f:
        f.write("MBERT SENTIMENT PERFORMANCE REPORT\n")
        f.write("==================================\n")
    ...
    print(f"\n>>> Hoàn thành! Kết quả đã được lưu tại {notebook_dir} và báo cáo tại {report_path}")
